...
class ChannelScanner extends EventEmitter {
    ...
    ScanChannel(channel) {
        ...
        this._channels[channel] =  new ChannelModule({ ChromeWsUrl: 'http://127.0.0.1:9222', Debug: this.Debug, settings: this._settings, SkipScan: this.SkipScan });
        this._channels[channel].on('ScanComplete', function(Status, ScanInterval) {
          if(_self._scanInterval[channel]) {
            ...
            if(!_self.Once) {
                console.log("Going to scan " + channel + " every " + ScanInterval + " seconds");
                ...
                if(Status == true) {
                    console.log("Insert " + channel + " into lineup");
