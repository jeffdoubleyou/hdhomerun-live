...
function decode_pkt(pkt, msg) {
	...
	while (pos < (msg.len + 4)) {
        ...
        //console.log(msg, "TAG: " + tag, "TAGLEN: " + tag_len);
		switch (tag) {
		...
		case (tags.error_message.value):
			...
			break;
		case (tags.device_auth_str.value):
			var gs_val = pkt.toString('ascii', pos, pos +
			    tag_len);
			pos += tag_len;
			msg.device_auth_str = gs_val;
			break;
		case (tags.lineup.value):
			var gs_val = pkt.toString('ascii', pos, pos +
			    tag_len);
			pos += tag_len;
			msg.lineup = gs_val;
...
function encode_tlv(tlv_obj, buf, offset) {
    ...
    //console.log("########## TAG NAME:", tlv_obj.tag);
    ...
    case (tags.base_url.value):
		...
		var tmpbuf = Buffer.concat([buf1, new Buffer('')]);
